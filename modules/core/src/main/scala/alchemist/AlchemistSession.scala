...
object AlchemistSession {
  ...
  def sendTestString(_testString: String = ""): this.type = {
    ...
    if (checkIfConnected) {
      ...
      val (responseString, sendOverhead, receiveOverhead): (String, Overhead, Overhead) =
        driver.sendTestString(testString)
  ...
  def getMatrixHandle(mat: DistributedMatrix, name: String = ""): MatrixHandle = {
    ...
    if (checkIfConnected) {
      val (mh, sendOverhead, receiveOverhead): (MatrixHandle, Overhead, Overhead) =
        driver.sendMatrixInfo(name, mat.numRows, mat.numCols)
  ...
  def sendIndexedRowMatrix(mat: IndexedRowMatrix): MatrixHandle = {
    ...
    if (checkIfConnected) {
      ...
      mat.rows.mapPartitionsWithIndex { (idx, part) =>
        ...
        workers foreach (w => {
          if (verbose)
            println(s"Spark executor ${idx}: Connecting to Alchemist worker ${w._2.ID} at ${w._2.address}:${w._2.port}")
          if (wc.connect(w._2.ID, w._2.hostname, w._2.address, w._2.port)) {
            if (verbose)
              println(
                s"Spark executor ${idx}: Connected to Alchemist worker ${w._2.ID} at ${w._2.address}:${w._2.port}, sending data ..."
              )
  ...
  def getIndexedRowMatrix(mh: MatrixHandle): IndexedRowMatrix = {
    ...
    if (checkIfConnected) {
      ...
      val mat: IndexedRowMatrix = new IndexedRowMatrix(spark.sparkContext.union(rowIndices.mapPartitionsWithIndex {
        (idx, part) =>
          {
            ...
            var tempRows: scala.collection.mutable.Map[Int, Array[Double]] =
              scala.collection.mutable.Map.empty[Int, Array[Double]]
            rowIndicesArray foreach (r => tempRows += (r.toInt -> Array.fill[Double](mh.numCols.toInt)(0.0)))

            workers foreach (w => {
              if (verbose)
                println(
                  s"Spark executor ${idx}: Connecting to Alchemist worker ${w._2.ID} at ${w._2.address}:${w._2.port}"
                )
              if (wc.connect(w._2.ID, w._2.hostname, w._2.address, w._2.port)) {
                if (verbose)
                  println(
                    s"Spark executor ${idx}: Connected to Alchemist worker ${w._2.ID} at ${w._2.address}:${w._2.port}, requesting data ..."
                  )
                val (_tempRows, workerSendOverheads, workerReceiveOverheads) =
                  wc.getIndexedRows(mh, rowIndicesArray, tempRows, idx)
                tempRows = _tempRows
                sendOverheads = sendOverheads :+ workerSendOverheads
                receiveOverheads = receiveOverheads :+ workerReceiveOverheads
              }
