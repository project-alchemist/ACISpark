...
//  }
  ...
  def getMatrixInfo: MatrixHandle = {
    ...
    (0 until numGridRows * numGridCols).foreach(
      _ => gridArray = gridArray + (messageBuffer.getShort -> Array(messageBuffer.getShort, messageBuffer.getShort))
    )
  ...
  def readByte: Byte = {
    ...
    if (code != Datatype.Byte.value) {
      ...
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.Byte.label}"
  ...
  def readChar: Char = {
    ...
    if (code != Datatype.Char.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.Char.label}"
      throw new InconsistentDatatypeException(message)
    }
  ...
  def readShort: Short = {
    ...
    if (code != Datatype.Short.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.Short.label}"
      throw new InconsistentDatatypeException(message)
    }
  ...
  def readInt: Int = {
    ...
    if (code != Datatype.Int.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.Int.label}"
      throw new InconsistentDatatypeException(message)
    }
  ...
  def readLong: Long = {
    ...
    if (code != Datatype.Long.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.Long.label}"
      throw new InconsistentDatatypeException(message)
    }
  ...
  def readFloat: Float = {
    ...
    if (code != Datatype.Float.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.Float.label}"
      throw new InconsistentDatatypeException(message)
    }
  ...
  def readDouble: Double = {
    ...
    if (code != Datatype.Double.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.Double.label}"
      throw new InconsistentDatatypeException(message)
    }
  ...
  def readString: String = {
    ...
    if (code != Datatype.String.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.String.label}"
      throw new InconsistentDatatypeException(message)
    }
  ...
  def readParameter: ParameterValue = {
    ...
    if (code != Datatype.Parameter.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.Parameter.label}"
      throw new InconsistentDatatypeException(message)
    }
  ...
  def readLibraryID: LibraryID = {
    ...
    if (code != Datatype.LibraryID.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.LibraryID.label}"
      throw new InconsistentDatatypeException(message)
    }
  ...
  def readMatrixID: MatrixID = {
    ...
    if (code != Datatype.MatrixID.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.MatrixID.label}"
      throw new InconsistentDatatypeException(message)
    }
  ...
  def readWorkerID: Short = {
    ...
    if (code != Datatype.WorkerID.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.WorkerID.label}"
      throw new InconsistentDatatypeException(message)
    }
  ...
  def readWorkerInfo: WorkerInfo = {
    ...
    if (code != Datatype.WorkerInfo.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.WorkerInfo.label}"
      throw new InconsistentDatatypeException(message)
    }
  ...
  def readIndexedRow: IndexedRow = {
    ...
    if (code != Datatype.IndexedRow.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.IndexedRow.label}"
      throw new InconsistentDatatypeException(message)
    }
  ...
  def readMatrixInfo: MatrixHandle = {
    ...
    if (code != Datatype.MatrixInfo.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.MatrixInfo.label}"
      throw new InconsistentDatatypeException(message)
    }
  ...
  def readMatrixBlock(readData: Boolean = true): MatrixBlock = {
    ...
    if (code != Datatype.MatrixBlock.value) {
      messageBuffer.asInstanceOf[Buffer].position(messageBuffer.asInstanceOf[Buffer].position - 1)
      val message: String =
        s"Actual datatype ${Datatype.withValue(code).label} does not match expected datatype ${Datatype.MatrixBlock.label}"
      throw new InconsistentDatatypeException(message)
    }
    ...
    val size: Long = math.ceil(1.0 * ((rows(1) - rows(0)) / rows(2)) + 1).toLong * math
      .ceil(1.0 * ((cols(1) - cols(0)) / cols(2)) + 1)
      .toLong
  ...
  def putParameter(p: ParameterValue): this.type = {
    ...
    p match {
      case Parameter(n: String, v: Byte) =>
        writeString(p.asInstanceOf[Parameter[Byte]].name).writeByte(p.asInstanceOf[Parameter[Byte]].value)
      case Parameter(n: String, v: Short) =>
        writeString(p.asInstanceOf[Parameter[Short]].name).writeShort(p.asInstanceOf[Parameter[Short]].value)
      case Parameter(n: String, v: Int) =>
        writeString(p.asInstanceOf[Parameter[Int]].name).writeInt(p.asInstanceOf[Parameter[Int]].value)
      case Parameter(n: String, v: Long) =>
        writeString(p.asInstanceOf[Parameter[Long]].name).writeLong(p.asInstanceOf[Parameter[Long]].value)
      case Parameter(n: String, v: Float) =>
        writeString(p.asInstanceOf[Parameter[Float]].name).writeFloat(p.asInstanceOf[Parameter[Float]].value)
      case Parameter(n: String, v: Double) =>
        writeString(p.asInstanceOf[Parameter[Double]].name).writeDouble(p.asInstanceOf[Parameter[Double]].value)
      case Parameter(n: String, v: Char) =>
        writeString(p.asInstanceOf[Parameter[Char]].name).writeChar(p.asInstanceOf[Parameter[Char]].value)
      case Parameter(n: String, v: String) =>
        writeString(p.asInstanceOf[Parameter[String]].name).writeString(p.asInstanceOf[Parameter[String]].value)
      case Parameter(n: String, v: MatrixID) =>
        writeString(p.asInstanceOf[Parameter[MatrixID]].name).writeMatrixID(p.asInstanceOf[Parameter[MatrixID]].value)
  ...
  def writeIndexedRow(index: Long, length: Long, values: Array[Double]): this.type =
    putDatatype(Datatype.IndexedRow.value).putIndexedRow(index, length, values)
  ...
  def print: this.type = {
    ...
    while (!eom) {
      ...
      currentDatatype match {
        ...
        case Datatype.MatrixInfo.value =>
          data = data.concat(s" ${readMatrixInfo.toString(space + "                                ")}")
        case Datatype.MatrixBlock.value =>
          data = data.concat(s" ${readMatrixBlock(true).toString(space + "                                 ")}")
