...
import java.io.{
    PrintWriter, FileOutputStream, OutputStream, IOException,
...
class DataOutputStream(ostream: OutputStream) extends JDataOutputStream(ostream) {
  ...
  def sendDoubleArray(buf: Array[Double]) {
    try {
      ...
      case e: IOException => println(s"alchemist.io.DataOutputStream: IO Exception thrown when writing array of Doubles");
    }
  }
  ...
  def sendBoolean(value: Boolean): this.type = {
    try {
      ...
      case e: IOException => println(s"alchemist.io.DataOutputStream: IO Exception thrown when writing Boolean '$value'");
    }
    this
  }
